...
def preprocessing():
    ...
    num_pid_repetitions = int(combined_df['situation_num'].max()) # finding how many times each entry of pid needs to be replicated so pid for each row of processed df_raw can be added
    pid_df = pd.DataFrame()
    pid_df['pid'] = None
